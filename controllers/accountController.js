...
exports.showLogin = (req, res) => {
    ...
    const emailOrUsername = req.cookies.emailOrUsername || '';
    const password = req.cookies.password || '';
    const rememberMe = req.cookies.rememberMe || false;

    res.render('login', { layout: '', error: req.flash('error'), emailOrUsername, password, rememberMe })
}
    ...
    passport.authenticate('local', (err, user, info) => {
        ...
        req.logIn(user, (err) => {
            ...
            if (req.body.rememberMe) {
                res.cookie('emailOrUsername', req.body.emailOrUsername);
                res.cookie('password', req.body.password);
                res.cookie('rememberMe', true)
            } else {
                res.clearCookie('emailOrUsername');
...
exports.resetPassword = async (req, res, next) => {
    try {
        ...
        req.flash('success', 'Mật khẩu của bạn đã được đặt lại thành công. Vui lòng đăng nhập.');
        res.redirect('/login');
